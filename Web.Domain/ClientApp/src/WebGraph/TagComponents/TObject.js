        ...
        Handle_GetAppBreadcrumbDetails: function (_BreakCrumbDict) {
            ...
            return __NewKeyValueList.map((__Item, __Index) => {
                return __Index + 1 === __NewKeyValueList.length ? (
                    ...
                    <Link
                        underline={"none"}
                        style={{ color: "primary.contrastText", textDecoration: "none" }}
                        href={__Item.Value}
                        onClick={(event) => __This.Handle_AClick(event, __Item.Value)}
                        onMouseEnter={(__Event) => { __Event.target.style.color = "primary.main" }}
                        onMouseOut={(__Event) => { __Event.target.style.color = "primary.contrastText" }}
                    >
