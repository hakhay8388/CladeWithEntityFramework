...
GenericWebGraph.Init = function (_OnComplete) {
    ...
    GenericWebGraph.SetInitializer(function (_ResponseData) {
            ...
            .then((_Event1) => {
                ...
                import("../GenericWebController/CommandListenerGraph/cCommandListenerGraph")
                    .then((_Event2) => {
                        ...
                        GenericWebGraph.InitUser(_ResponseData, _OnComplete);
                    });
            });
...
GenericWebGraph.Reinit = function () {
    GenericWebGraph.SetRenitializer(function (_ResponseData) {
        GenericWebGraph.InitUser(_ResponseData, function () {
            window.GoMainPage();
        });
    });
}
...
GenericWebGraph.InitUser = function (_ResponseData, _OnComplete) {
    var __Language = GenericWebGraph.GetCommandByNameInCommandArray(_ResponseData, "Language");
    GenericWebGraph.Managers.LanguageManager.HandleSetActiveLanguage(__Language.Data);
...
GenericWebGraph.SetRenitializer = function (_Initializer) {
...
GenericWebGraph.SetInitializer = function (_Initializer) {
    var __LanguageCode = window.GetLanguageCodeFromUrl();
    if (__LanguageCode.length != 2) {
        __LanguageCode = "";
    }
